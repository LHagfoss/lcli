...
use tabled::{
    builder::Builder,
    settings::{
        object::{Columns, Rows}, Alignment, Modify, Padding, Panel,
        Style,
    },
};
...
pub fn handle_about_command() {
    ...
    builder.push_record([&version]);
    builder.push_record([&author]);
    builder.push_record([website]);
    builder.push_record([github]);

    let mut table = builder.build();
    table
        .with(Style::rounded())
        .with(Panel::header(
            "LCLI - a stupid project by me".cyan().bold().to_string(),
        ))
